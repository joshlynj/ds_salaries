...
def create_us_only(ds_df):
    ...
    us_ft_res_df = ft_only_df \
        .drop(ft_only_df[ft_only_df['company_location'] != "US"].index)
    us_ft_res_df = us_ft_res_df \
        .drop(us_ft_res_df[us_ft_res_df['employee_residence'] != "US"].index)
    return us_ft_res_df


def create_highest_pay(us_ft_res_df):
    ''' Function takes in a dataframe
        Returns a dataframe that is grouped by the Job Title and Includes:
        Salary in USD, Employment Type, and Company Location'''
    highest_pay = us_ft_res_df.groupby("job_title")["salary_in_usd",
                                                    "employment_type",
                                                    "company_location"] \
        .max().sort_values(by='salary_in_usd', ascending=False)
...
def create_lowest_pay(us_ft_res_df):
    ''' Function takes in a dataframe
    Returns a dataframe that is grouped by the Job Title and Includes:
    Salary in USD, Employment Type, and Company Location'''
    lowest_pay = us_ft_res_df.groupby("job_title")["salary_in_usd",
                                                   "employment_type",
                                                   "company_location"]\
...
def add_job_count(us_ft_res_df):
    '''Function takes in a dataframe
    Returns a dataframe with a count of the jobs'''
    us_ft_res_df['job_count'] = us_ft_res_df\
        .groupby('job_title')['job_title']\
        .transform('count')
    return us_ft_res_df


def create_most_common(us_ft_res_df):
    ''' Function takes in a dataframe 
        Returns a dataframe that shows '''
    most_common_df = us_ft_res_df\
        .groupby("job_count",
...
def create_remote_df(us_ft_res_df):
    '''Function takes in a dataframe
        Returns a data frame with only the remote ratio and company size'''
    remote_df = us_ft_res_df[['remote_ratio',
                              "company_size"]]\
